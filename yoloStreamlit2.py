...
uploaded_file = st.file_uploader("파일을 업로드하세요", type=["jpg", "jpeg", "png", "avi"])
...
if uploaded_file:
    ...
    if input_type == "이미지" and uploaded_file.type in ["image/jpeg", "image/png"]:
    ...
    elif input_type == "동영상" and uploaded_file.type == "video/avi":
        # 동영상 예측
        st.video(uploaded_file)
        
        with st.spinner("Predicting on video..."):
            temp_video_path = os.path.join(UPLOAD_DIR, "uploaded_video.avi")
        ...
        # 결과 동영상 표시
        result_video_path = results[0].save_dir / "uploaded_video.avi"
